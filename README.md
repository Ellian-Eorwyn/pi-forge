...
- **`single`** (the default) — one image-capable model on `llms` doing everything, with the whole window that model serves and no delegation. `forge_delegate` is not registered in this setup: without a second backend it would fall back to the primary, which on a one-slot server is the interactive session's own model, so a delegate call would evict that session's prefix cache and cost more than the context it saves.
...
- From inside the agent: `/backend` shows the active setup, `/backend use <name>` switches, and `/backend on|off` toggles delegation. The two modes have one-word commands — **`/single`** for the default (one model, the full window it serves, no delegation) and **`/multi`** for the multi-model setup (`distributed-parallel`, with `forge_delegate` enabled). `/single` turns *every* multi-model knob (second bulk lane, verify lane, delegation, compaction offload) back off in one step, which is also what you want when the second GPU is powered down or re-cabled. `/parallel` remains as an alias for `/multi`. Skills pick up the change on their next call; the interactive model, its window, and whether `forge_delegate` exists update on your next session.
- From the terminal: `node <package>/scripts/backends.mjs [list|show|use <name>|single|multi|apply|delegation on|off]`.

A setup's `primary.contextTokens` may be the literal number of tokens or `"auto"`. All three shipped setups use `"auto"`, which reads the per-slot window from the deployment's state API when the setup is applied and falls back to the built-in constant when that API cannot be reached — so a setup declares exactly what its backend serves, and follows a backend that is later given a bigger window without needing an edit here. Declaring more than the server serves is not cosmetic: a skill sends a prompt this client believes fits and reads the server's rejection as the model failing. (`secondary.contextTokens` stays a literal; the probe reads the primary's chat endpoint only.) The same read clamps `scheduling.backgroundSlot` when the backend has fewer slots than the pin names. An existing `backends.json` written before `"auto"` is upgraded to it on the next apply, but only where the value is still byte-equal to the built-in default — any other number was chosen deliberately and is left alone.
