...
test("a counted budget is exhausted on the call that reaches the declared limit", () => {
	withLedger((path) => {
		for (let call = 1; call < DECLARED_BUDGETS.nyt.calls; call += 1) recordProviderSpend("nyt", { options: { path, now: NOW } });
...
test("a reported balance outranks the local count", () => {
	withLedger((path) => {
		...
		const plenty = new Headers({ "x-ratelimit-limit-usd": "0.1", "x-ratelimit-remaining-usd": "0.05", "x-ratelimit-cost-usd": "0.001" });
		for (let call = 0; call < 1000; call += 1) recordProviderSpend("openalex", { headers: plenty, options: { path, now: NOW } });
		...
		const spent = new Headers({ "x-ratelimit-limit-usd": "0.1", "x-ratelimit-remaining-usd": "0.0005", "x-ratelimit-cost-usd": "0.001" });
...
test("a rate-limit header set is read only when it says something", () => {
	...
	const core = reportedRateLimit(new Headers({ "x-ratelimit-limit": "10", "x-ratelimit-retry-after": "2026-07-31T19:34:03+0000" }));
